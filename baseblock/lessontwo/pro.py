...
class Keywords():
    def __init__(self, vars_):
        ...
        self.error = colored('Ошибка. Вы создали одну или несколько переменных с именем встроенной функции:',
        ...
        self.information = '\nИсправьте имена переменных и повторите попытку' \
                           ...
                           + colored(
            'https://colab.research.google.com/drive/1kUQVE_vTJZEiNJ5yt0Zgg0MRVYslY4b4?usp=sharing', color='blue')
...
def task1(tmp, text=None):
    assert tmp.rnd_array.shape == (3, 4) or tmp.rnd_array.dtype == np.int or (
                tmp.rnd_array.max() >= 38 and tmp.rnd_array.min() <= 15), 'Неправильный размер массива или содержание массива'
